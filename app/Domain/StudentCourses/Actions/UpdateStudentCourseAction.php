<?php

namespace App\Domain\StudentCourses\Actions;

use App\Domain\StudentCourses\DTO\StoreStudentCourseDTO;
use App\Domain\StudentCourses\DTO\UpdateStudentCourseDTO;
use App\Domain\StudentCourses\Models\StudentCourse;
use Exception;
use Illuminate\Support\Facades\DB;

class UpdateStudentCourseAction
{
    /**
     * @param UpdateStudentCourseDTO $dto
     * @return StudentCourse
     * @throws Exception
     */
    public function execute(UpdateStudentCourseDTO $dto): StudentCourse
    {
        DB::beginTransaction();
        try {
            $student_course = $dto->getStudentCourse();
            $student_course->title = $dto->getTitle();
            $student_course->update();
        } catch (Exception $exception) {
            DB::rollBack();
            throw $exception;
        }
        DB::commit();
        return $student_course;
    }
}
