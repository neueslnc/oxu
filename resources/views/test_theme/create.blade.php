                ...
                <div class="modal-dialog modal-fullscreen">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <button id="save" type="button" class="btn btn-primary">Saqlash</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script src="{{ url('vue/component.js?13') }}"></script>
...
//
//     vm.$data.datas.filter(item => item.id == item_id)[0].writing = tinymce.get("mytextarea1").getContent();
//
//     $('#exampleFullScreenModalAnswer').modal('hide');
// });
...
function send_data() {
    ...
    if($("#name").val() == ''){
        ...
        return error_noti(msg=`Mavzuni belgilang.`);
    }
    ...
    for (const iterator of vm.$data.datas) {
        ...
        if(iterator.question_name == ""){
            ...
            return error_noti(msg=`Savol sarlavhasini belgilang - ${ i }`);
        }

        if(iterator.waiting_seconds == ""){

            return error_noti(msg=`Savolning vaqtini belgilang - ${ i }`);
        }
        ...
        if (iterator.type == "variant") {
            ...
            if(iterator.variants.length < 2){
                ...
                return error_noti(msg=`Savol - ${ i }, birdan ko'p variant bering.`);
            }
            ...
            if(iterator.variants.filter(item => item.correct == true).length == 0){
                ...
                return error_noti(msg=`Savol - ${ i }, To'g'ri javob ko'rsating.`);
            }
            ...
            for (const iterator1 of iterator.comparisons) {
                ...
                if (iterator1.block.block_left.name == "" || iterator1.block.block_right.name == "") {
                    ...
                    return error_noti(msg=`Savol - ${ i } qism- ${ b }, Solishtirma nomini ko'rsating.`);
                }
    ...
    $.ajax('{{ route('test_theme.store') }}', {
        ...
        success: function (data, status) {
            ...
            if (status == 'success') {
                succes_noti(title="Операция выполнена успешно", msg="Тесты сохранены");
                vm.$data.datas = [];
            }
        },
        error: function (jqXhr, textStatus, errorMessage) {

            return error_noti(msg=jqXhr.responseJSON.message);
        }
    });
}
