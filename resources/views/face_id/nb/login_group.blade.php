...
@section('script_include_header')
    ...
    <script defer src="{{ asset('script_group.js?23') }}"></script>
                                                    ...
                                                    <div class="border p-4 rounded">
                                                        ...
                                                        <div class="form-body">
                                                            <form method="POST" action="{{ route('login_post') }}" class="row g-3">
                                                                ...
                                                                <div class="col-md-12">
                                                                    <label for="direction_id" class="form-label">Guruh</label>
                                                                    <div class="input-group">
                                                                        <select id="group_id" class="single-select form-select" name="group_id">
                                                                ...
                                                                <div class="col-12">
                                                                    <div class="d-grid">
                                                                        <button id="submit" type="button" class="btn btn-primary"><i class="bx bxs-lock-open"></i>Выбрать</button>
                                                                    </div>
                                                                </div>
                                                            </form>
                                                        </div>
...
@push('scripte_include_end_body')
    ...
    <script>
        ...
        $('#submit').on('click', function name() {
            $.ajax('{{ route('supervisor_exams.get_students_face_id') }}', {
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}",
                ...
                success: async function (data, status) {
                    ...
                    // await clearInterval(window.interval);
                    //
            ...
            return await $.ajax('{{ route('supervisor_exams.set_pc_active') }}', {
                ...
                success : function (data, status) {
                    ...
                    if (data.status == 200) {
                        ...
                        window.deactive_student.push({
                            id : data.data.user_id,
                            fio : window.data.find(item => item.id == data.data.user_id).fio
                        });

                        window.faceMatcher._labeledDescriptors = window.faceMatcher._labeledDescriptors.filter(item => item._label != window.data.find(item => item.id == data.data.user_id).fio);
